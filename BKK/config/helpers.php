...
function allow_role(array $allowed_roles) {
    if (!is_logged_in()) {
        // Redirect unauthenticated users to a relevant login page based on allowed roles
        if (in_array('alumni', $allowed_roles, true)) {
            redirect('../Login/LoginSiswa/siswa-alumni-login.php');
        }
        if (in_array('admin', $allowed_roles, true)) {
            redirect('../Login/LoginAdmin/admin-login.php');
        }
        if (in_array('manajemen', $allowed_roles, true)) {
            redirect('../Login/LoginManagement/management-login.php');
        }
        if (in_array('recruiter', $allowed_roles, true)) {
            redirect('../Login/LoginUserLain/pengguna-lain-login.html');
        }
...
function base_url($path = '') {
    return '/PROJECT-BKK-main/BKK/' . ltrim($path, '/');
}
